...
export class CliRunner {
  ...
  public async execute(
    ...
    if (showOutput) {
      ...
      this.outputChannel.appendLine(
        `[${new Date().toLocaleTimeString()}] Executing: ripp ${args.join(' ')}`
      );
      this.outputChannel.appendLine('---');
    }
  ...
  /**
   * Extract generated file paths from CLI output
   * Returns array of paths mentioned in the output
   */
  public extractGeneratedPaths(output: string): string[] {
    const paths: string[] = [];

    // Common patterns for generated paths
    const patterns = [
      /Created:\s+(.+)/gi,
      /Generated:\s+(.+)/gi,
      /Output:\s+(.+)/gi,
      ...
      /RIPP Packet:\s+(.+)/gi,
      /Handoff MD:\s+(.+)/gi,
      /Moved:\s+(.+)\s+→\s+(.+)/gi, // Migration output
      /✓\s+(.+\.(?:yaml|yml|json|md))/gi // Files with checkmarks
    ];

    for (const pattern of patterns) {
      let match;
      while ((match = pattern.exec(output)) !== null) {
        // Extract the path (last captured group for "Moved" pattern, first for others)
        const pathMatch = match[match.length - 1].trim();
        if (pathMatch && !paths.includes(pathMatch)) {
