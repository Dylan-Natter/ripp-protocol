  ...
  try {
    ...
    console.log('\nTest 2: Single-file markdown packaging');
    {
      ...
      try {
        ...
        assert(
          !content.includes('## Packaging Information'),
          'Does not contain packaging info section'
        );
        assert(content.length > 0, 'Output file not empty');
      } catch (error) {
        console.log(`  Error: ${error.message}`);
    ...
    console.log('\nTest 5: Deterministic output');
    {
      ...
      try {
        execSync(`node ${cliPath} package --in ${testPacketPath} --out ${output1Path} --single`, {
          encoding: 'utf8',
          stdio: 'pipe'
        });
        ...
        execSync(`node ${cliPath} package --in ${testPacketPath} --out ${output2Path} --single`, {
          encoding: 'utf8',
          stdio: 'pipe'
        });
